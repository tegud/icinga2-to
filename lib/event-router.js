...
module.exports = function(app, config) {
    ...
    function filter(routeFilter, event) {
        ...
        logging.logInfo('Event matches filter', { host: event.data.host.name, service: event.data.service.name, checkStatus: event.data.state.status, checkStatusType: event.data.state.status, checkAttempt: event.data.state.attempt, filter: routeFilter });

...
module.exports = function(app, config) {
    ...
    const eventStreams = config.streams.map(streamConfig => {
        ...
        return new require('./event-stream')(app, {
            baseRequest: baseRequest,
            queue: streamConfig.queue,
            types: streamConfig.types,
            filter: streamConfig.filter,
            eventName: streamConfig.event,
            baseUrl: icingaUrl
        })
    });
    ...
    return {
        start: () => Promise.all(eventStreams.map(eventStream => eventStream.start()))
